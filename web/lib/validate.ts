/**
 * Shared input validation, output escaping, and rate-limiting helpers.
 */
...
export function isFuture(datetime: string | number | Date, minLeadMs = 15 * 60_000): boolean {
  const t = new Date(datetime).getTime();
  if (!Number.isFinite(t)) return false;
  return t > Date.now() + minLeadMs;
}

export function isWithinBookingWindow(datetime: string | number | Date, maxAheadDays = 90): boolean {
  const t = new Date(datetime).getTime();
  if (!Number.isFinite(t)) return false;
...
export function isBusinessHours(datetime: string | number | Date, tz = "America/Chicago"): { ok: boolean; reason?: string } {
  ...
  if (!(hour >= 8 && hour < 18)) return { ok: false, reason: "Please choose a time between 8am and 6pm Central." };
  return { ok: true };
}

export function truncate(s: unknown, max = 2000): string {
  if (s === null || s === undefined) return "";
  const str = String(s);
...
export function checkRateLimit(ip: string, { max = 5, windowMs = 60_000 }: { max?: number; windowMs?: number } = {}): {
