/**
 * Forensic trail for admin mutations (audit M2-6).
 *
 * Every destructive admin endpoint should call logAdminAction() AFTER the
 * mutation completes. Best-effort: logging failures never block the response.
 *
 * What gets recorded:
 ...
 *
 * Raw bearer tokens and raw IPs are never stored.
 */
...
function hashWithSalt(value: string | null | undefined): string | null {
  ...
  return crypto.createHash("sha256").update(`${value}:${IP_SALT}`).digest("hex").slice(0, 32);
}

function extractActorHash(req: Request): string | null {
...
export async function logAdminAction(req: Request, fields: AuditFields): Promise<boolean> {
